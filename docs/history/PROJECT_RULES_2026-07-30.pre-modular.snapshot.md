...
- **Guardrail:** Route diagnostic prose through a shared word-wrap helper using `min(98, Console.WindowWidth - 1)`, stable continuation prefixes, and a long-token fallback so prose stays inside the 99-column SMART table footprint. Split dense counter summaries into deliberate semantic rows so a final key/value pair cannot become an orphaned continuation. Never insert unbordered prose between a table header separator and its data rows; render legends as merged boxed rows with exactly the same outer width as the table. Validate new terminal output at multiple widths with a real virtual-terminal emulator; string-content assertions alone are insufficient for UI layout.
- **Files affected:** `DiskHealth.ps1`, `tests\AtaTelemetry.Tests.ps1`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
- **Validation:** `60/90/118` PowerShell fixtures and compact long-token regression passed. Temporary `pyte 0.8.2` virtual terminals rendered the final merged legend, segmented header, and sample data row as `8 source rows -> 8 rendered rows` at `100/120/150` columns; every table row is exactly 99 columns. Earlier live NEOS evidence also confirmed the Seagate context content at 120 columns. The grouped evidence layout keeps `SMART Error Log` with its value instead of wrapping `Log=0` alone.
